...
export default {
    ...
    mounted(){
        ...
        if(this.$store.state.user.savedPaths[0]){ //dohvaćanje spremljenih puteva i odabranog puta pri učitavanju
    ...
    methods:{
        getPath(arr){ //dohvaćanje odabranog puta
            let headers = {
        ...
        getSavedPaths(arr){ //dohvaćanje spremljenih puteva
            let headers = {
                'Content-Type': 'application/json'
            }
        ...
        deletePath(){ //brisanje spremljenog puta
    ...
    computed:{
        pathDataLoaded(){ //provjera postoji li odabrani put
