			...
			<div class="dropdown-container drow">
				...
				<span v-if="collegeSelected">
					...
					<div class="dropdown dcol xs-9 m-10 l-7	">
						<v-select 
							...
							:reduce="smjer => smjer._id" 
							v-model="$store.state.selectedOptions.smjerId" 
							placeholder="Odaberite željeni smjer"
							@input="refresh('smjerId')"
						>
						</v-select>
					</div>
				</span>
				...
				<div class="dropdown dcol xs-9 m-10 l-7	">
					<v-select 
						...
						@input="refresh('zanimanjeId')"
					>
					</v-select>
				</div>

				<div class="dropdown-icon confirm-btn dcol offset-xs-1 xs-2 m-1 offset-l-2 l-1" @click="confirm()" v-if="!validator">Potvrdi</div>
				<div class="confirm-btn-disabled dcol offset-xs-1 xs-2 m-1 offset-l-2 l-1" v-if="validator">Potvrdi</div>
			</div>
	</div>
</template>
	...
	methods: {
		...
		refresh(id){ //reset pojedinog odabira
			...
			if(this.college == null){
				this.college = ''
				this.$store.state.selectedOptions.fakultetId = ""
			}
		}
	},
