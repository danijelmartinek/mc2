            ...
            <accordion :accordionID="'acc-' + generateID()" label="Smjerovi" width="200px">
                <div v-for="smjer in data.selectedCollege.smjerovi" :key="smjer._id">
                    <accordion :accordionID="'acc-' + generateID()" :label="smjer.naziv" width="220px">
                        <div class="infoElement">Smjer ima menzu: {{ ((smjer.prehrana == true) ? 'DA' : 'NE') }}</div>
                        <div class="infoElement">KOLEGIJI:
                            <div class="elementDivider"></div>
                            <div v-for="kolegij in smjer.kolegiji" :key="kolegij._id">
                                <accordion :accordionID="'acc-' + generateID()" :label="kolegij.naziv" width="240px">
                                    <div class="infoElementNested">SEMESTAR KOLEGIJA: {{ kolegij.semestar }}</div>
                                </accordion>
                                <div class="elementDivider"></div>
                            </div>
                        </div>
